...
func shouldSkipConfig(run, configRun, configSkip string) bool {
	if run != "" && configRun != "" && run != configRun {
		return true
	}
	target := run
	if target == "" {
	...
	if target != "" && configSkip != "" {
		for _, skip := range strings.Split(configSkip, ",") {
			if strings.TrimSpace(skip) == target {
				return true
			}
		}
	}
